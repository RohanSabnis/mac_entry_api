 ...
 */
avltree_t *
avltree_create(uint32_t key, void *data) {
 ...
 */
void
avltree_free_node(avltree_t *node) {
    if (node != 0x0) {
        node->key = 0;
        node->parent = 0x0;
        node->left_child = 0x0;
 ...
 * need to be searched.
 * @param  key  key of new AVL Tree node
 * @return 		a pointer points inorder successor
 */
static avltree_t *
minValueNode(avltree_t *node)
{
    avltree_t *current = node;
...
/**
 * Delete an AVL Tree node
 * @param node - AVL Tree root node
 * 		  key - key of node to be deleted
 * @return 		a pointer to new root
 */
avltree_t *
avltree_delete_node(avltree_t *root, uint32_t key) {
	// base case
	...
	if (key < root->key)
		root->left_child = avltree_delete_node(root->left_child, key);

	// If the key to be deleted is greater than the root's key,
	// then it lies in right subtree
	else if (key > root->key)
		root->right_child = avltree_delete_node(root->right_child, key);
	...
	else
	{
		...
		{
			avltree_t *temp = root->right_child;
			free(root);
			return temp;
		}
		else if (root->right_child == NULL)
		{
			avltree_t *temp = root->left_child;
			free(root);
			return temp;
		}
		...
		avltree_t* temp = minValueNode(root->right_child);
		...
		root->right_child = avltree_delete_node(root->right_child, temp->key);
	}
	return root;
};
 ...
 */
void
avltree_free_tree(avltree_t *node) {
    if (node == NULL) return;
    // Free left subtree
 ...
 */
uint32_t
avltree_get_key(avltree_t *node) {
 ...
 */
void *
avltree_get_data(avltree_t *node) {
 ...
 */
int
avltree_get_height(avltree_t *node, int current_level) {
 ...
 */
static avltree_t *
avltree_rotate_left(avltree_t *node) {
 ...
 */
static avltree_t *
avltree_rotate_right(avltree_t *node) {
 ...
 */
static avltree_t *
avltree_rotate_left_right(avltree_t *node) {
 ...
 */
static avltree_t *
avltree_rotate_right_left(avltree_t *node) {
 ...
 */
avltree_t *
avltree_get_root(avltree_t *node) {
    if ( node == NULL) return NULL;
 ...
 */
static int
avltree_get_balance_factor(avltree_t *node) {
    if (node == NULL) return 0;
 ...
 */
avltree_t *
avltree_insert(avltree_t *root, avltree_t *node) {
    if (root == NULL) {
        return node;
    }
 ...
 */
avltree_t *
avltree_find(avltree_t *root, uint32_t key) {
 ...
 */
void
avltree_show_tree(avltree_t *node) {
...
/**
 * Show current AVL Node structure
 * @param node any node of the AVL Tree
 */
void
avltree_show_node(avltree_t *node) {
    if (node != NULL) {
        printf("{Key: %u, Left: %u, Right: %u}\n", node->key,
        		node->left_child == NULL ? 0 : node->left_child->key,
        		node->right_child == NULL ? 0 : node->right_child->key);
    }
}
